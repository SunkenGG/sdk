...
public class MathUtils {
    ...
    static {
        // Setup for fast sine/cosine lookup table
        BF_SIN_BITS = 12; // Number of bits to use for the sine table (2^12 = 4096 entries)
    ...
    /**
     * Fast sine lookup using precomputed values.
     * @param radians The angle in radians
     * @return The sine of the angle
     */
    public static float sin(double radians) {
        radians = radians % BF_radFull;
        if (radians < 0) {
            radians += BF_radFull;
        }
        int index = (int) (radians * BF_radToIndex) & BF_SIN_MASK;
        return BF_sin[index];
    }

    /**
     * Fast cosine lookup using precomputed values.
     *
     * @param radians The angle in radians
